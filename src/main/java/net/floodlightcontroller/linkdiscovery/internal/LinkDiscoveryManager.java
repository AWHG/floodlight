...
IFloodlightModule, IInfoProvider {
	...
	protected void writeLinkToStorage(Link lt, LinkInfo linkInfo) {
		...
		String enc_id = SEAlgorithm.streamEnc(id, key, IV);
		rowValues.put(LINK_ID, enc_id);

		String enc_validTime = SEAlgorithm.streamEnc(linkInfo.getUnicastValidTime().toString(),key,IV);
		rowValues.put(LINK_VALID_TIME, enc_validTime);

		String srcDpid = lt.getSrc().toString();
		String enc_srcDpid = SEAlgorithm.streamEnc(srcDpid, key, IV);
		rowValues.put(LINK_SRC_SWITCH, enc_srcDpid);

		String enc_srcPort = SEAlgorithm.streamEnc(lt.getSrcPort().toString(), key, IV);
		rowValues.put(LINK_SRC_PORT, enc_srcPort);

		if (type == LinkType.DIRECT_LINK)
			rowValues.put(LINK_TYPE, SEAlgorithm.streamEnc("internal", key, IV));
		else if (type == LinkType.MULTIHOP_LINK)
			rowValues.put(LINK_TYPE, SEAlgorithm.streamEnc("external", key, IV));
		else if (type == LinkType.TUNNEL)
			rowValues.put(LINK_TYPE, SEAlgorithm.streamEnc("tunnel", key, IV));
		else
			rowValues.put(LINK_TYPE, SEAlgorithm.streamEnc("invalid", key, IV));

		String dstDpid = lt.getDst().toString();
		String enc_dstDpid = SEAlgorithm.streamEnc(dstDpid, key, IV);
		rowValues.put(LINK_DST_SWITCH, enc_dstDpid);


		rowValues.put(LINK_DST_PORT, SEAlgorithm.streamEnc(lt.getDstPort().toString(), key, IV));
